...
public class Employee {
    ...
    public static void main(String[] args) {
        List<Employee> res = filterEmployees(
                Employee.getSampleData(),
                s -> s.getSalary() > 70000 && s.getDepartment().equals("ENGINEERING")
        );

        List<String> names = Employee.getSampleData().stream()
                .map(s -> s.getName().toUpperCase())
                .collect(Collectors.toList());

        double totalSalary = Employee.getSampleData().stream()
